...
import { ArrowUpFromLine, Check, X, Loader2 } from "lucide-react";
...
export default function AdminWithdrawalsPage() {
  ...
  const [actionRow, setActionRow] = useState<AdminWithdrawal | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      showToast(err instanceof Error ? err.message : "Failed to load withdrawals", "error");
    } finally {
      setLoading(false);
    }
  }, [tab, showToast]);
  ...
  async function confirmAction() {
    ...
    try {
      if (actionType === "approve") {
        await approveWithdrawal(actionRow.id, {
          externalTransactionId: externalId || undefined,
          comment: comment || undefined,
        });
        showToast("Withdrawal approved");
      } else {
        await rejectWithdrawal(actionRow.id, reason);
  ...
  const tableRows = rows.map((r) => ({ ...r })) as Array<AdminWithdrawal & Record<string, unknown>>;
  ...
  return (
    <div className="mx-auto w-full space-y-6">
      <div className="flex items-start gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[var(--app-text-primary)]">Withdrawals</h1>
      ...
      <AdminDataTable
        ...
        columns={[
          ...
          { key: "paymentMethod", label: "Method" },
          {
            ...
            render: (r) => (
              <span className="max-w-[200px] truncate">
                {String((r.paymentDetails as Record<string, unknown>)?.note ?? "—")}
              </span>
            ),
          },
          {
          ...
          },
          {
            ...
            label: "Actions",
            render: (r) => {
              const row = r as AdminWithdrawal;
        ...
        {actionType === "approve" ? (
          <div className="space-y-3">
            <input
              ...
              placeholder="Payout transaction ID (optional)"
